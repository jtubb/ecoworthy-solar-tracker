...
Custom-firmware project for an off-the-shelf "Ecoworthy" dual-axis solar tracker board, plus an ESPHome bridge to Home Assistant via an ESP-01S. **Phases 0–4 are complete:** the STC15F2K60S2 runs a full custom tracker firmware — boot auto-zero, stall-to-stall calibration (max of extend/retract stroke, with extend stall-delay correction), relay-on-time position tracking, a menu UI (Track / Jog / Calibrate / Backlash / Settings / Version) with debounced resistor-ladder buttons and press-and-hold auto-repeat, EEPROM-persisted settings, differential sun-sensor pulse-tracking with per-axis duty-cycle limiting, a wind storm interlock (park-to-horizontal + resettable dwell), and a duty-cycle interlock with a bottom-right padlock indicator (forced cooldown after calibration). Phase 3 adds the **half-duplex single-wire bridge** to an ESP-01S running ESPHome (`tracker_bridge` external_component at `esphome/components/`): the ESP polls every 2 s, the STC replies with a `\xAA\x55`-framed CRC-8/SMBUS status packet, and az/el/wind/mode are published to Home Assistant. Firmware lives in `EcoWorthyFirmware/`; the ESP-side component lives at `esphome/components/tracker_bridge/` (canonical ESPHome lookup path) with reference YAMLs at `EcoWorthyFirmware/esphome/solar-tracker-{1,2}.yaml`. **Phase 4** adds a **multi-tracker ESP-NOW mesh**: one designated primary (the node with a physical wind sensor, `local_role: primary`) broadcasts AES-128-GCM encrypted WIND packets every 5 s; secondaries receive and forward the wind reading to their local STC over the existing UART bridge (`!wind=NN`). Every node also broadcasts TELEMETRY (az/el/wind/mode) and GATEWAY_HB (RSSI) every 5 s. The acting gateway — the lowest-MAC WiFi-up node in the `peers:` list — publishes per-peer HA entities from received TELEMETRY. Operator command buttons (`force_park`, `force_release`, `stop`) and RW config sliders (`wind_storm_mps`, `wind_release_mps`, `storm_dwell_min`, `track_thresh`) are wired on each node and can optionally target a remote peer via `peer_id:`. The mesh packet format is a 45-byte AAD-covered header (`type src_mac epoch ctr node_name`) + AES-128-GCM ciphertext + 8-byte tag; nonce is `src_mac(6) || epoch(2) || ctr(4)`. A 16-bit flash-persisted boot epoch prevents replay across power cycles. The `tracker_mesh_psk` secret (≥ 16 chars; SHA-256 truncated to 16 bytes for the AES key) must be identical on all nodes in the array. **Phase 5** adds a **night-park** mode: the STC autonomously transitions from `ST_TRACK` to `ST_NIGHT_PARK` when the per-sensor sun-ADC average stays below `night_park_dark_thr` (default 30, range 5–200) for `night_park_dark_min` minutes (default 30, range 1–120), seeks a configurable east-flat target (`night_park_az_pct=100`, `night_park_el_pct=50` by default; goto reuses the storm-park axis machinery), and wakes back to `ST_TRACK` once any sensor reads above `dark_thr + 20` for 6 consecutive ~10s samples. Storm always preempts. Tunable via 5 HA sliders + 5 LCD menu pages + 5 new cfg-protocol field IDs (`0x05`–`0x09`: `CFG_F_NIGHT_PARK_EN/AZ/EL/MIN/THR`). A `!sim_dark on/off` UART debug command forces the dark state for bench testing without physical sensor occlusion.
...
- STC firmware lives in `EcoWorthyFirmware/` (SDCC + stcgal). ESP-01S YAMLs live in `EcoWorthyFirmware/esphome/`. Build/flash commands are documented in `EcoWorthyFirmware/README.md`.
